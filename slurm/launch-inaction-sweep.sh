#!/bin/bash
# Launch a focused inaction_penalty sweep on the SLURM cluster.
# All frosty-sweep-27 hyperparameters locked; only inaction_penalty swept.
#
# Usage:
#   bash slurm/launch-inaction-sweep.sh        # 4 parallel agents (default)
#   bash slurm/launch-inaction-sweep.sh 2      # override: 2 agents
...
echo "=== Creating Inaction Penalty W&B Sweep ==="
set +e
SWEEP_OUTPUT=$(env/bin/python -m wandb sweep configs/inaction_sweep.yaml 2>&1)
SWEEP_EXIT=$?
set -e
echo "$SWEEP_OUTPUT"
...
if [[ -z "$SWEEP_ID" ]]; then
    ...
    echo "Try running 'env/bin/python -m wandb sweep configs/inaction_sweep.yaml' manually."
...
echo "=== Inaction Penalty Sweep Launched ==="
