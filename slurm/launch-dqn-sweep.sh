#!/bin/bash
# Launch a DQN hyperparameter sweep on the SLURM cluster.
# This script runs LOCALLY (not via sbatch). It:
#   1. Creates a W&B sweep from configs/dqn_sweep.yaml
#   2. Launches 8 parallel SLURM GPU jobs as sweep agents
#
# Usage:
#   bash slurm/launch-dqn-sweep.sh
...
echo "=== Creating DQN W&B Sweep ==="
set +e
SWEEP_OUTPUT=$(env/bin/python -m wandb sweep configs/dqn_sweep.yaml 2>&1)
SWEEP_EXIT=$?
set -e
echo "$SWEEP_OUTPUT"
...
if [[ $SWEEP_EXIT -ne 0 ]]; then
    ...
    echo "Try running manually: env/bin/python -m wandb sweep configs/dqn_sweep.yaml"
    exit 1
fi
...
if [[ -z "$SWEEP_ID" ]]; then
    ...
    echo "Try running 'env/bin/python -m wandb sweep configs/dqn_sweep.yaml' manually and note the sweep ID."
    echo "Then launch agents with: sbatch --array=1-${N_AGENTS} slurm/slurm-sweep-dqn.sh <sweep_id>"
    exit 1
fi
...
JOB_ID=$(sbatch --array=1-${N_AGENTS} --export=SWEEP_ID="${SWEEP_ID}" slurm/slurm-sweep-dqn.sh | awk '{print $NF}')

echo "=== DQN Sweep Launched ==="
...
echo "  cat slurm/logs/dqn_sweep_*          # job logs"
