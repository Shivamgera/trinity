...
set +e
SWEEP_OUTPUT=$(wandb sweep configs/dqn_sweep.yaml 2>&1)
SWEEP_EXIT=$?
set -e
echo "$SWEEP_OUTPUT"
...
if [[ $SWEEP_EXIT -ne 0 ]]; then
    ...
    echo "Try running manually: wandb sweep configs/dqn_sweep.yaml"
    exit 1
fi
...
if [[ -z "$SWEEP_ID" ]]; then
    ...
    echo "Try running 'wandb sweep configs/dqn_sweep.yaml' manually and note the sweep ID."
